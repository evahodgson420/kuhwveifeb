...
async function grantDealAccess(channel, userId) {
    ...
    if (channel.isThread?.()) {
        // For the parent channel: only allow viewing (so the thread is visible),
        // but do NOT grant SendMessages in the parent — buyer stays in the thread only.
        ...
        if (parentChannel && typeof parentChannel.permissionOverwrites?.edit === 'function') {
            await parentChannel.permissionOverwrites.edit(userId, {
                ...
                console.error(`Could not set parent perms for ${userId} on ${parentChannel.id}`, err.message);
            });
        }
