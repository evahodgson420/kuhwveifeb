...
const priceCache = require('./core/priceCache');
...
const ticketHandlers = {
    ...
    clientpaid_: require('./tickets/clientPaid'),
    clientpaidaccept_: require('./tickets/clientPaid'),
    clientpaiddecline_: require('./tickets/clientPaid')
};
...
client.once(Events.ClientReady, async (c) => {
    ...
    c.user.setPresence({
        status: 'idle',
        activities: [{ name: 'OrbitTrade.org', type: ActivityType.Watching }]
    });

    // Start LTC price cache (refreshes every 60s, used by /balance and ticket creation)
    priceCache.startRefresh();
                ...
                try {
                    ...
                    for (const utxo of unspent) {
                        ...
                        try {
                            ...
                            // DM: 0/6 confirmation notification
                            const [pendingUserRows] = await db.query(
                                'SELECT discord_id FROM users WHERE id = ?',
                                [userId]
                            );
                            await notifyUserDm(
                                client,
                                pendingUserRows[0]?.discord_id,
                                {
                                    color: appConfig.brand.color,
                                    title: 'Orbit Trade | Deposit Detected (0/6)',
                                    description:
                                        `> Amount: \`${parseFloat(amount).toFixed(8)}\` LTC\n` +
                                        `> TXID: \`${txHash}\`\n` +
                                        `> Status: **Unconfirmed — awaiting 6 confirmations**`,
                    ...
                    for (const deposit of pendings) {
                        ...
                        try {
                            ...
                            await notifyUserDm(
                                ...
                                {
                                    color: appConfig.brand.color,
                                    title: 'Orbit Trade | Deposit Confirmed (6/6)',
                                    description:
                                        `> Amount: \`${parseFloat(deposit.amount).toFixed(8)}\` LTC\n` +
                                        `> TXID: \`${deposit.txid}\`\n` +
                                        `> Status: **Confirmed (6/6) — balance credited**`,
                                    timestamp: new Date().toISOString()
                                }
                            );
                        } catch (txErr) {
